    ...
    new_users = [
        {
            'id'              : 1,
            'name'            : '강태규',
            'email'           : 'eugene3516@gmail.com',
            'profile'         : 'test profile',
            'hashed_password' : hashed_password
        }, {
            'id'              : 2,
            'name'            : '박재호',
            'email'           : 'pks328@gmail.com',
            'profile'         : 'test profile',
            'hashed_password' : hashed_password
        }
    ]
...
def teardown_function():
    ...
    engine.execute(text("TRUNCATE users"))
    engine.execute(text("TRUNCATE tweets"))
    engine.execute(text("TRUNCATE users_follow_list"))
...
def test_login(api):
    resp = api.post(
        '/login',
        data         = json.dumps({'email' : 'eugene3516@gmail.com', 'password' : 'test password'}),
